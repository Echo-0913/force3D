octree = function (points, x1, y1, z1, x2, y2, z2) {
    function defaultX(d) {
        return d.x;
    }
    function defaultY(d) {
    ...
    var x = defaultX, y = defaultY, z = defaultZ, compat;
    ...
    function octree(data) {
        var d, fx = functor(x), fy = functor(y), fz = functor(z), xs, ys, zs, i, n, x1_, y1_, z1_, x2_, y2_, z2_;
        if (x1 != null) {
    ...
    octree.extent = function (_) {
        ...
        if (_ == null) x1 = y1 = z1 = x2 = y2 = z2 = null; else x1 = +_[0][0], y1 = +_[0][1], x2 = +_[1][0],
            y2 = +_[1][1], z1 = +_[0][2], z2 = +_[1][2];
        return octree;
    };
    octree.size = function (_) {
        if (!arguments.length) return x1 == null ? null : [x2 - x1, y2 - y1, z2 - z1];
        if (_ == null) x1 = y1 = z1 = x2 = y2 = z2 = null; else x1 = y1 = z1 = 0, x2 = +_[0], y2 = +_[1], z2 = +_[2];
        return octree;
    };
    return octree;
};
...
function octreeFind(root, x, y, z, x0, y0, z0, x3, y3, z3) {
    ...
    (function find(node, x1, y1, z1, x2, y2, z2) {
        ...
        var children = node.nodes, xm = (x1 + x2) * .5, ym = (y1 + y2) * .5, zm = (z1 + z2) * .5,  right = x >= xm, below = y >= ym, long = z >= zm;
